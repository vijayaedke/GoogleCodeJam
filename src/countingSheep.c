...
void fillHash(int *hashmap, int mul){
...
unsigned long findCount(int n){
    unsigned long mul=1;
    int i=1;
    int hashmap[N]={0};
    while(i<=100){
        mul = (unsigned long) ( n*i);
        fillHash(hashmap, mul);
        if(checkHash(hashmap)==1)
            return mul;
        i++;
    }//while
    return mul;
